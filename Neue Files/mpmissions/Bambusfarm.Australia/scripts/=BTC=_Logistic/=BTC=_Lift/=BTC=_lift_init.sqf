...
if (BTC_def_hud == 1) then 
{
	...
	BTC_arrow_up   = "scripts\=BTC=_Logistic\=BTC=_Lift\img\arrow_up_ca.paa";
	BTC_arrow_down = "scripts\=BTC=_Logistic\=BTC=_Lift\img\arrow_down_ca.paa";
	BTC_complete   = "scripts\=BTC=_Logistic\=BTC=_Lift\img\objective_complete_ca.paa";
	BTC_incomplete = "scripts\=BTC=_Logistic\=BTC=_Lift\img\objective_incomplete_ca.paa";
};
BTC_cargo      = ObjNull;
//Functions

//if (__GETC__(donator_level) == 3) then
//{
  BTC_lift_check =
  {
  	private ["_rel_pos"];
  ...
  BTC_attach_cargo =
  {
    ...
    _chopper = vehicle player;
  	...
  	_chopper vehicleChat format ["%1 wurde angehängt", _name_cargo];
  ...
  BTC_detach_cargo =
  {
  	...
  	vehicle player vehicleChat format ["%1 wurde abgesetzt", _name_cargo];
  ...
  [] spawn
  {
  	...
  	if (BTC_def_pip == 1) then {player addAction [("<t color=""#ED2744"">" + ("Camera AN\AUS") + "</t>"),BTC_dir_action, [[],BTC_l_camera], -9, false, false, "", "typeOf (vehicle player) in BTC_l_def_veh_pip"];};
  	player addAction [("<t color=""#ED2744"">" + ("Fahrzeug anhängen") + "</t>"),BTC_dir_action, [[],BTC_attach_cargo], 9, true, false, "", "[] call BTC_lift_check"];
  	player addAction [("<t color=""#ED2744"">" + ("Fahrzeug abhängen") + "</t>"),BTC_dir_action, [[],BTC_detach_cargo], -9, true, false, "", "BTC_lifted == 1"];
  	player addEventHandler ["Respawn", 
  	{
  		[] spawn 
  		{
  			...
  			BTC_lifted   = 0;
  			if (BTC_def_hud == 1) then {player addAction [("<t color=""#ED2744"">" + ("Hud AN\AUS") + "</t>"),BTC_dir_action, [[],{if (BTC_Hud_Cond) then {BTC_Hud_Cond = false;} else {BTC_Hud_Cond = true;_hud = [] spawn BTC_fnc_hud;};}], -8, false, false, "", "(vehicle player) isKindOf ""Helicopter"" && driver (vehicle player) == player"];};
  			if (BTC_def_pip == 1) then {player addAction [("<t color=""#ED2744"">" + ("Camera AUS\AUS") + "</t>"),BTC_dir_action, [[],BTC_l_camera], -9, false, false, "", "typeOf (vehicle player) in BTC_l_def_veh_pip"];};
  			player addAction [("<t color=""#ED2744"">" + ("Fahrzeug anhängen") + "</t>"),BTC_dir_action, [[],BTC_attach_cargo], 9, true, false, "", "[] call BTC_lift_check"];
  			player addAction [("<t color=""#ED2744"">" + ("Fahrzeug abhängen") + "</t>"),BTC_dir_action, [[],BTC_detach_cargo], -9, true, false, "", "BTC_lifted == 1"];
  		};
  	}];
