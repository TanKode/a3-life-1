...
//if((_unit getVariable ["restrained",TRUE])) exitWith {hint "Die Person muss gefeselt sein!"};//victims hands must be rape-taped
...
if(!([false,"scalpel",1] call life_fnc_handleInv)) exitWith {hint "Sie haben kein Scalpel"};
if(life_inv_kidney >= 2) exitWith {hint "Sie koennen nicht so viele Organe gebrauchen!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"kidney",1] call life_fnc_handleInv)) exitWith {hint "Sie haben nicht genung platz fuer die Organe!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
...
[player,"niere"] call life_fnc_globalSound;