...
while{true} do
{
    ...
    //detector
	_speedtrapDe = "speedtrapdetector";
    _has_radar_detector = 0;
    _var_speedtrapdetector = M_CONFIG(getText,"VirtualItems",_speedtrapDe,"displayName");
    _value_speedtrapdetector = ITEM_VALUE(_speedtrapDe);
    ...
    if((count _speedtraps) > 0 && _has_radar_detector == 1 && _speed > 0) then
    {
        ...
        if( (_last_detector_time == 0) || ((_last_detector_time + _blocktime_detector) < time) ) then
        {
            ...
            if((count _trap_object) > 0) then
            {
                ...
                [["", _trap_object select 0, "Radarfalle", time],"king_fnc_addTrackingInfosToPlayers",player,false] spawn life_fnc_MP;
            ...
            if(_distance < _trap_distance && _speed > (_speedlimit + _trap_speed_tolerance)) then
            {
                ...
                if(!_visible) then
                {
                    ...
                    if( (_last_traptime == 0) || ((_last_traptime + _blocktime_trap) < time) ) then
                    {
                        ...
                        //hint format["Sie wurden geblitzt! speed=%1 limit=%2 driver=%3", _trapped_speed, floor(_speedlimit), _trapped_driver];
                        //_customType = format["Geblitzt %1km/h bei %2km/h",_trapped_speed, _speedlimit];
                        //[[getPlayerUID player, name player, "customtype", 10000, _customType],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
                        _trapped_infos = [];
                        ...
                        [_trapped_infos, _trap_object] spawn king_fnc_trapped;
                    };
                };
            };
        };
