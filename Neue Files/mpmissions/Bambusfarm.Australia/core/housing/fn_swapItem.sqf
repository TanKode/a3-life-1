...
if (_pid!=getplayerUID player) exitWith { hint parseText "<t color='#FF0000' size='1'>Das Iventar wurde geschlossen druch das Anti - Dupe system!";curHouse setVariable ["inUse",[false,""],true];closeDialog 0; };
...
if (_pid==getplayerUID player) then {
	switch (_mode) do {
		case 0 : {
			...
			if (_selClassname1 == "") exitWith {life_action_inUse = false;}; // duping fix};
			_house = cursorTarget;
			...
			_index = -1;
			{
				...
				if (_x select 0 == _selClassname1 AND _selClassname1 != "B_supplyCrate_F" AND _selClassname1 != "Box_IND_AmmoVeh_F" AND _selClassname1 != "Box_IND_WpsSpecial_F" AND _selClassname1 != "Box_IND_Grenades_F") then {
					...
					if (_canBeRemoved) then {
						...
						hint format ["Sie haben nicht genung Platz in ihrem Inventar fuer: %1",_item select 1];
					};
				};
			} forEach _content;
		...
		case 1 : {
			life_action_inUse = true; //duping fix
			if (_selClassname2 == "") exitWith {life_action_inUse = false;}; // duping fix};
			_house = cursorTarget;
			_content = _house getVariable "content";
			...
			if ((_boxslotsused+1) <= curHouseSpace) then {
				...
				if (true) exitWith {hint "Ihr Haus ist voll!"};
			};
