	...
	if((count _actSafezone) > 0) then
	{
		if((player distance (getMarkerPos (_actSafezone select 0))) > _actSafezone select 3) then
		{
			if(alive player) then {
				hint parseText format["<t color='#FF0000'><t size='2'>Safezone Info</t></t><br/>Du verlaesst die Safezone: %1!",_actSafezone select 1];
			};
	...
	} else {
		{
			if((player distance (getMarkerPos (_x select 0))) < _x select 3 && playerSide == _x select 2) exitWith {
				...
				hint parseText format["<t color='#FF0000'><t size='2'>Safezone Info</t></t><br/>Du betrittst eine Safezone: %1!",_x select 1];
			};
			sleep 1;
