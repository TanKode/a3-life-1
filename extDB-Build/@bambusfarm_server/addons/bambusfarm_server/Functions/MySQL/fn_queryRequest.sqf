...
*/
private["_uid","_side","_query","_query2","_query4","_return","_queryResult","_queryResult2","_queryResult4","_qResult","_handler","_thread","_loops","_returnCount"];
...
_query = switch(_side) do
{
	...
	case civilian: {_returnCount = 12; format["SELECT playerid, name, cash, bankacc, adminlevel, Karma, civ_licenses, arrested, civ_gear FROM players WHERE playerid='%1'",_uid];};
...
// Level
_query4 = switch(_side) do {
   case west: {_returnCount = 1; format["SELECT donatorlevel FROM levelsys WHERE playerid='%1'",_uid];};
   case civilian: {_returnCount = 1; format["SELECT donatorlevel FROM levelsys WHERE playerid='%1'",_uid];};
   case independent: {_returnCount = 1; format["SELECT donatorlevel FROM levelsys WHERE playerid='%1'",_uid];};
};
waitUntil{sleep (random 0.3); !DB_Async_Active};
_queryResult4 = [_query4,2] call DB_fnc_asyncCall;
...
diag_log "------------- Client Level Request -------";
diag_log format["::::::::::::: _queryResult4 (Level): %1",_queryResult4];
