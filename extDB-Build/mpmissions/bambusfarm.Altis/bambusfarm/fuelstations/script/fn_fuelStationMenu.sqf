...
[] spawn {
	waitUntil{!bambusfarm_action_inUse};
	bambusfarm_fuelstation_lockVeh = [];
	if(!isNull bambusfarm_fuelstation_fueltruck) then {
		bambusfarm_fuelstation_fueltruck setVariable["fueltruck_in_use",false,true];
		bambusfarm_fuelstation_fueltruck = objNull;
	};
	bambusfarm_fuelstation_amount = 0;
	bambusfarm_fuelstation_fitamount = 0;
...
if(_mode == "FUELTRUCK") then {
	...
	bambusfarm_fuelstation_fueltruck = _station;
};
if(_fuelInUse) exitWith { hint format["Tanklaster bereits in benutzung!"]; bambusfarm_action_inUse = false; };
...
if((count _veh) < 1) exitWith { hint format["No Vehicle"]; bambusfarm_action_inUse = false; };
...
bambusfarm_fuelstation_price = ([((((call sell_array) select _indFuel) select 1) / 200) * _pricemod, 2] call BIS_fnc_cutDecimals) max 3;
...
{
	_type = typeOf _x;
	_info = [_type] call bambusfarm_fnc_fetchVehInfo;
	_owner = "";
	if(_x in bambusfarm_vehicles) then { _owner = " (Besitzer)"; };
	_kap = [_type] call bambusfarm_fnc_vehFuelCap;
	...
	if((_x != bambusfarm_fuelstation_fueltruck) && _type in bambusfarm_fuelstation_cfgTrucks) then {
		_kap = [_type + "_TRAILER"] call bambusfarm_fnc_vehFuelCap;
