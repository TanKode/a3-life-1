...
//if(playerside != independent)then{ [] spawn life_fnc_DelGWH; };
...
if(!isNull _killer && {_killer != _unit} && {side _killer != west} && {alive _killer}) then {
	if(vehicle _killer isKindOf "LandVehicle") then {
		...
		[[-100,1,life_Karma],"life_fnc_KarmaNeg",_killer,false] call life_fnc_MP;
	} else {
...
if(side _killer == west && playerSide != west) then {
	...
	if(!life_use_atm && {life_TASCHENGELD > 0}) then {
		[format[localize "STR_Cop_RobberDead",[life_TASCHENGELD] call life_fnc_numberText],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		life_TASCHENGELD = 0;
	};
};
