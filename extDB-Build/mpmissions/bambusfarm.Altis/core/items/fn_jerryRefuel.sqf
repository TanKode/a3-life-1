...
if(!([false,"fuelF",1] call bambusfarm_fnc_handleInv)) exitWith {};
...
if(!alive player) exitWith {};
if(bambusfarm_interrupted) exitWith {bambusfarm_interrupted = false; titleText[localize "STR_NOTF_ActionCancel","PLAIN"];};
...
switch (true) do
{
	case (_vehicle isKindOF "LandVehicle"):
	{
		...
		{
			[[_vehicle,(Fuel _vehicle) + 0.5],"bambusfarm_fnc_setFuel",_vehicle,false] spawn bambusfarm_fnc_MP;
		}
			else
		{
	...
	case (_vehicle isKindOf "Air"):
	{
		if(!local _vehicle) then
		{
			[[_vehicle,(Fuel _vehicle) + 0.2],"bambusfarm_fnc_setFuel",_vehicle,false] spawn bambusfarm_fnc_MP;
		}
			else
		{
	...
	case (_vehicle isKindOf "Ship"):
	{
		if(!local _vehicle) then
		{
			[[_vehicle,(Fuel _vehicle) + 0.35],"bambusfarm_fnc_setFuel",_vehicle,false] spawn bambusfarm_fnc_MP;
		}
			else
		{
...
[true,"fuelE",1] call bambusfarm_fnc_handleInv;