...
_ind = [_data,bambusfarm_illegal_items] call TON_fnc_index;
if(_ind != -1 && ([west,getPos player,100] call bambusfarm_fnc_nearUnits)) exitWith {titleText["This is an illegal item and cops are near by, you cannot dispose of the evidence","PLAIN"]};
if(player != vehicle player) exitWith {titleText["You cannot remove an item when you are in a vehicle.","PLAIN"]};
if(!([false,_data,(parseNumber _value)] call bambusfarm_fnc_handleInv)) exitWith {hint "Couldn't remove that much of that item, maybe you don't have that amount?"};
_type = [_data,0] call bambusfarm_fnc_varHandle;
