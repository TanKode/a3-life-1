...
*/
if(bambusfarm_action_inUse) exitWith {hint localize "STR_NOTF_ActionInProc"};
disableSerialization;
private["_nearVehicles","_control"];
_nearVehicles = nearestObjects [getMarkerPos (_this select 3),["Car","Truck"],25];

bambusfarm_chopShop = (_this select 3);
//Error check
...
_control = ((findDisplay 39400) displayCtrl 39402);
{
	if(alive _x) then {
		...
		_ind = [_className,(call bambusfarm_garage_sell)] call TON_fnc_index;
		
		if(_ind != -1 && count crew _x == 0) then {
			_price = ((call bambusfarm_garage_sell) select _ind) select 1;
