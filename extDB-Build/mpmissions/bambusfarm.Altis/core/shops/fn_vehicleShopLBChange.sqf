...
[_basePrice] call bambusfarm_fnc_numberText,
[round(_basePrice * 1.5)] call bambusfarm_fnc_numberText,
...
lbClear _ctrl;
_colorArray = [_className] call bambusfarm_fnc_vehicleColorCfg;

for "_i" from 0 to count(_colorArray)-1 do {
	if((_colorArray select _i) select 1 == (bambusfarm_veh_shop select 2)) then {
		_temp = [_className,_i] call bambusfarm_fnc_vehicleColorStr;
		_ctrl lbAdd format["%1",_temp];
		_ctrl lbSetValue [(lbSize _ctrl)-1,_i];
	};
};

if(_className in (__GETC__(bambusfarm_vShop_rentalOnly))) then {
	ctrlEnable [2309,false];
} else {
	if(!(bambusfarm_veh_shop select 3)) then {
