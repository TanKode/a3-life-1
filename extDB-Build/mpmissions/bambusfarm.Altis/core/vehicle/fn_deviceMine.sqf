...
bambusfarm_action_inUse = true; //Lock out the interaction menu for a bit..
_weight = [_vehicle] call bambusfarm_fnc_vehicleWeight;
if((_weight select 1) >= (_weight select 0)) exitWith {hint localize "STR_NOTF_DeviceFull"; bambusfarm_action_inUse = false;};
...
if(_item == "") exitWith {hint "Bad Resource?"; bambusfarm_action_inUse = false;};
_vehicle setVariable ["mining",true,true]; //Lock the device
[_vehicle,"bambusfarm_fnc_soundDevice",true,false] spawn bambusfarm_fnc_MP; //Broadcast the 'mining' sound of the device for nearby units.

bambusfarm_action_inUse = false; //Unlock it since it's going to do it's own thing...
...
while {true} do {
	...
	_weight = [_vehicle] call bambusfarm_fnc_vehicleWeight;
	_sum = [_item,15,_weight select 1,_weight select 0] call bambusfarm_fnc_calWeightDiff; //Get a sum base of the remaining weight..
	if(_sum < 1) exitWith {titleText[localize "STR_NOTF_DeviceFull","PLAIN"];};
	_itemWeight = ([_item] call bambusfarm_fnc_itemWeight) * _sum;
	...
	if(local _vehicle) then {
		...
		[[_vehicle,(fuel _vehicle)-0.04],"bambusfarm_fnc_setFuel",_vehicle,false] spawn bambusfarm_fnc_MP;
	};
