...
if (bambusfarm_BANK < parseNumber (_selectedOffer select 2)) exitWith {hint "Du hast nicht genug Geld dabei!";};
_seller = [_selectedOffer select 0] call bambusfarm_fnc_CAH_getPlayerObj;
_selectedOffer = [_selectedOffer select 0,_selectedOffer select 1,parseNumber (_selectedOffer select 2)];
[[player,_seller,_selectedOffer],"TON_fnc_SAH_Transaction",false,false] spawn bambusfarm_fnc_mp;
