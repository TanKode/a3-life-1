...
if(bambusfarm_ATMhacked == 1) exitWith { hint "Die Zentralbank wurde bereits gehackt! Es zur Zeit kein Hackangriff möglich"; };
...
_RANDHACKMONEY = [3500,250000] call bambusfarm_fnc_randomRound; // GELD was erhackt wurde
...
//// PROZESSBAR START /////////////////
	...
	_uip = uiNameSpace getVariable "bambusfarm_progress";
...
//
			...
			if(_chancetohackATM > 65) then 
			{
				...
				for "_i" from 1 to 20 do
				{
					...
					sleep 1;
					if(player distance _lappi > 2) exitWith { hint "Du hast dich zu weit entfernt! Das wars für heute!"; bambusfarm_action_inUse = false; };
				};

				bambusfarm_BANK = bambusfarm_BANK + _RANDHACKMONEY;
				
				hint format["Du konntest dir %1 Euro auf dein Konto überweisen!",_RANDHACKMONEY];
				sleep 3;
				bambusfarm_ATMhacked = 1;
				...
				//
				[[0,format["Die Zentralbank wurde gehackt!!"]],"bambusfarm_fnc_broadcast",west,false] spawn bambusfarm_fnc_MP;
				sleep 1;
				...
				if(_chanceWantedAdd < 65) then {
					...
					[[getPlayerUID player,name player,"1002"],"bambusfarm_fnc_wantedAdd",false,false] spawn bambusfarm_fnc_MP;
					[[0,format["Der Hacker hat Spuren hinterlassen! %1 hat in den Computer der Zentralbank eingegriffen!!",name player]],"bambusfarm_fnc_broadcast",west,false] spawn bambusfarm_fnc_MP;
				} else {
					// Der Hacker hat keine Spuren hinterlassen, und kommt nicht auf die Wantedliste! 
					// Glück gehabt! 
					[[0,format["Der Hacker hat keine Spuren hinterlassen!"]],"bambusfarm_fnc_broadcast",west,false] spawn bambusfarm_fnc_MP;
				};
			}
				else
			{
			...
			// Das wars für den Hacker!
						...
						[[getPlayerUID player,name player,"1003"],"bambusfarm_fnc_wantedAdd",false,false] spawn bambusfarm_fnc_MP;
						[[0,format["%1 hat versucht den ZentralbankComputer zu hacken!",name player]],"bambusfarm_fnc_broadcast",west,false] spawn bambusfarm_fnc_MP;
