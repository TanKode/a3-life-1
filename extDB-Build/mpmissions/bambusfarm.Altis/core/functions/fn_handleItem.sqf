...
_details = [_item] call bambusfarm_fnc_fetchCfgDetails;
...
if(_bool) then
{
	switch((_details select 6)) do
	{
		...
		case "CfgVehicles":
		{
			...
			{ 
				{[_x,true,true,false,true] spawn bambusfarm_fnc_handleItem; } foreach _items;
			};
		};
				...
				else
			{
				switch(_details select 5) do
				{
							...
							else
						{
								...
								else
							{
									...
									else
								{
									...
									if(!isNil {_items}) then
									{
										{[_x,true,false,false,true] spawn bambusfarm_fnc_handleItem;} foreach _items;
									};
								};
							};
						};
					...
					case 201:
					{
						...
						{
							private["_type"];
							_type = [_item,201] call bambusfarm_fnc_accType;
							if(_ongun) then
							{
								switch (_type) do
								{
					...
					case 301:
					{
						...
						{
							private["_type"];
							_type = [_item,301] call bambusfarm_fnc_accType;
					...
					case 101:
					{
						...
						{
							private["_type"];
							_type = [_item,101] call bambusfarm_fnc_accType;
