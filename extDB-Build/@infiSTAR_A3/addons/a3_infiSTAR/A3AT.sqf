...
infiSTAR_MAIN_CODE = "
	...
	fnc_LBDblClick_RIGHT = {
		...
		{
			if(_click == 'Give All Liscenses')then{{missionnamespace setVariable[(_x select 0),true];} foreach bambusfarm_licenses;};
			_click call fnc_adminLog;
		};
		if(_click in infiSTAR_AltisLifeMoney)then
		{
			switch (_click) do {
				case '50K Cash':{['cash',50000] call fnc_Altisbambusfarm_cash;};
				case '100K Cash':{['cash',100000] call fnc_Altisbambusfarm_cash;};
				case '250K Cash':{['cash',250000] call fnc_Altisbambusfarm_cash;};
				case '500K Cash':{['cash',500000] call fnc_Altisbambusfarm_cash;};
				case '750K Cash':{['cash',750000] call fnc_Altisbambusfarm_cash;};
				case '1.150K Cash':{['cash',1150000] call fnc_Altisbambusfarm_cash;};
				case '2.500K Cash':{['cash',2500000] call fnc_Altisbambusfarm_cash;};
				case '5.000K Cash':{['cash',5000000] call fnc_Altisbambusfarm_cash;};
				case '10.000K Cash':{['cash',10000000] call fnc_Altisbambusfarm_cash;};
				case '50K Bank':{['bank',50000] call fnc_Altisbambusfarm_cash;};
				case '100K Bank':{['bank',100000] call fnc_Altisbambusfarm_cash;};
				case '250K Bank':{['bank',250000] call fnc_Altisbambusfarm_cash;};
				case '500K Bank':{['bank',500000] call fnc_Altisbambusfarm_cash;};
				case '750K Bank':{['bank',750000] call fnc_Altisbambusfarm_cash;};
				case '1.150K Bank':{['bank',1150000] call fnc_Altisbambusfarm_cash;};
				case '2.500K Bank':{['bank',2500000] call fnc_Altisbambusfarm_cash;};
				case '5.000K Bank':{['bank',5000000] call fnc_Altisbambusfarm_cash;};
				case '10.000K Bank':{['bank',10000000] call fnc_Altisbambusfarm_cash;};
			};
			_click call fnc_adminLog;
		};
		"; }; infiSTAR_MAIN_CODE = infiSTAR_MAIN_CODE + "
			...
			FNC_VIRTUAL_ITEMSTHREAD = {
				...
				(findDisplay MAIN_DISPLAY_ID displayCtrl 65466) ctrlSetEventHandler['ButtonClick','
					...
					if(_msg != '''')then
					{
						...
						[''[true,''+str _click+'',''+str _value+''] call bambusfarm_fnc_handleInv;'',SELECTED_TARGET_PLAYER] call admin_d0_target;
	...
	fnc_Altisbambusfarm_cash = {
		...
		if(_state == 'cash')then
		{
			...
			systemChat format['Cash %1K - %2K has been added.',bambusfarm_TASCHENGELD/1000,_amount/1000];
		};
		if(_state == 'bank')then
		{
			bambusfarm_BANK = bambusfarm_BANK + _amount;
			systemChat format['Bank %1K - %2K has been added.',bambusfarm_BANK/1000,_amount/1000];
		};
	};
	admin_showinfo = {
		_obj = cursortarget;
	...
	fnc_ReviveTarget = {
		...
		if(infiSTAR_MOD == 'AltisLife')then
		{
			...
			[[profileName],'bambusfarm_fnc_revived',_target,false] call bambusfarm_fnc_MP;
		}
		else
		{
	...
	infiSTAR_A3Heal = {
		if(!isNil 'bambusfarm_TASCHENGELD')then{bambusfarm_thirst = 100;bambusfarm_hunger = 100;};
		if(infiSTAR_MOD == 'Epoch')then{
		...
		if(_veh isKindOf 'AllVehicles')exitWith{
			if((player distance _veh < 12) || ((_veh == vehicle player) && (vehicle player != player)))then
			{
				...
				if(_locked isEqualTo 2)then
				{
					...
					{
						_veh lock 0;
						[[_veh,0],'bambusfarm_fnc_lockVehicle',_veh,false] call bambusfarm_fnc_MP;
						[[_veh,'car_unlock'],'bambusfarm_fnc_say3D',nil,false] spawn bambusfarm_fnc_MP;
					}
					else
					{
						_veh lock false;
				...
				else
				{
					...
					{
						_veh lock 2;
						[[_veh,2],'bambusfarm_fnc_lockVehicle',_veh,false] call bambusfarm_fnc_MP;
						[[_veh,'car_lock'],'bambusfarm_fnc_say3D',nil,false] spawn bambusfarm_fnc_MP;
					}
					else
					{
						_veh lock true;
	...
	infiSTAR_A3Invulnerability = {
		fnc_A3_stopGod =
		{
			...
			if((infiSTAR_MOD == 'AltisLife') && (!isNil 'bambusfarm_fnc_HandleDamage'))then
			{
			...
			if(!isNil 'obambusfarm_fnc_tazed')then{bambusfarm_fnc_tazed = obambusfarm_fnc_tazed;};
		};
		...
		if(infiSTAR_A3GodRun == 0)then
		{
			...
			if(!isNil 'bambusfarm_fnc_tazed')then
			{
				if(isNil 'obambusfarm_fnc_tazed')then
				{
					obambusfarm_fnc_tazed = bambusfarm_fnc_tazed;
				};
				bambusfarm_fnc_tazed = {};
			};
			infiSTAR_A3InvulnerabilityLoop = [] spawn {
				while {1==1} do
				{
					...
					if(!isNil 'bambusfarm_thirst')then{
