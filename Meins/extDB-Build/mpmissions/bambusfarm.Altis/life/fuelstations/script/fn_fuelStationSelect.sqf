...
if(_isTrailer == 1) then {
	_fitFuel = [(typeOf _veh) + "_TRAILER"] call life_fnc_vehFuelCap;
	_fitFuel = floor (_fitFuel - (_veh getVariable["fueltruck_fuel",0]));
} else {
	_fitFuel = [typeOf _veh] call life_fnc_vehFuelCap;
...
if(isNull life_fuelstation_fueltruck) then {
	_cashFit = ( floor (life_BANK / life_fuelstation_price)) min _fitFuel;
