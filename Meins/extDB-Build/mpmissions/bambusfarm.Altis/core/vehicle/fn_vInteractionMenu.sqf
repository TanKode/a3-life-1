...
if(playerSide == west) then {
    ...
    if(playerSide in [west]) then {
    	...
    	_Btn7 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_crush;";
    } else {
...
if(playerSide == civilian)then
{
	if(_curTarget isKindOf "Ship") then {
		...
		_Btn2 buttonSetAction "[] spawn RiskYourlife_fnc_pushObject; closeDialog 0;";
		...
		if(typeOf (_curTarget) in ["C_Kart_01_Blu_F","C_Kart_01_Red_F","C_Kart_01_Fuel_F","C_Kart_01_Vrana_F"]) then {
			...
			_Btn2 buttonSetAction "player moveInDriver RiskYourlife_vInact_curTarget; closeDialog 0;";
			...
			_Btn2 buttonSetAction "RiskYourlife_vInact_curTarget setPos [getPos RiskYourlife_vInact_curTarget select 0, getPos RiskYourlife_vInact_curTarget select 1, (getPos RiskYourlife_vInact_curTarget select 2)+0.5]; closeDialog 0;";
	...
	if(typeOf _curTarget == "O_Truck_03_device_F") then {
		...
		_Btn3 buttonSetAction "[RiskYourlife_vInact_curTarget] spawn RiskYourlife_fnc_deviceMine";
		if(!isNil {(_curTarget getVariable "mining")} OR !local _curTarget && {_curTarget in RiskYourlife_vehicles}) then {
...
if(playerSide == independent)then
{
    ...
    if((!isNull _curTarget) && (_curTarget isKindOf "air")) then
    {
    	...
    	_Btn1 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_repCar;";
    };
