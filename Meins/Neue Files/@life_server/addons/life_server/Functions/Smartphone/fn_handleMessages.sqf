...
_pos = [_this,4,[],[[]]] call BIS_fnc_param;
...
switch(_type) do
{
	...
	case 0:
	{
		...
		_query = format["smartphoneInsertMessages:%1:%2:%3:%4:%5",_pid,_toID,_msg,_fromName,_toName];
	...
	case 7:
	{
		[[_msg,name _player,7,_pos],"TON_fnc_clientMessage",east,false] spawn life_fnc_MP;	
	};
	//auftrag request
	case 8:
	{
		...
		[[_msg,name _player,8],"TON_fnc_clientMessage",_to,false] spawn life_fnc_MP;	
	};
	
	case 9:
	{
		[[_msg,name _player,9],"TON_fnc_clientMessage",true,false] spawn life_fnc_MP;
	};
};