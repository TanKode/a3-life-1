		...
		{
			{
				...
				[_x,_veh] spawn 
				{
					...
					_unload = _veh addaction [("<t color=""#ffcc00"">") + (format ["Abladen %1",getText (configFile >> "cfgVehicles" >> typeof _obj >> "displayName")]) + "</t>",BTC_dir_action,[[_veh,_obj],BTC_l_unload],7,true,false,"","true"];
					_sleep = time + 15;
...
BTC_l_select =
{
	...
	if (count _array > 0) then 
	{
		...
		{hint "You can not load this object";BTC_cargo_selected = objNull;} else {hint parseText format ["%1 ausgewählt<br/>CR: %2",getText (configFile >> "cfgVehicles" >> typeof BTC_cargo_selected >> "displayName"),[BTC_cargo_selected] call BTC_get_rc];};
	};
};
BTC_l_load =
{
	...
	if (!isNull _veh) then
	{
		...
		if (BTC_cargo_selected distance _veh < 15 && speed _veh < 2 && _cond) then
		{
			...
			if (Alive BTC_cargo_selected && Alive _veh && BTC_cargo_selected distance _veh < 15 && speed _veh <= 2 && speed _veh >= -2) then
			{
				...
				hint format ["Sie haben &1 in %2 geladen",_obj_name,_veh_name];
			} else {hint format ["%1 wurde nicht eingeladen",_obj_name];};
		} else {if (!_cond) then {hint "Das Fahrzeug ist voll!";} else {hint "Das Objekt was Sie einladen wollen ist zu weit vom Fahrzeug entfernt!";};};
	};
};
BTC_l_unload =
{
	...
	if (_id != -1) then
	{
		...
		hint format ["%1 wurde aus %2 ausgeladen",_obj_name,_veh_name];
		sleep 3;
		hint format ["%1 wurde ausgeladen",_obj_name,_veh_name];
...
BTC_l_drag =
{
	...
	if (format ["%1", _drag getVariable "BTC_cannot_drag"] == "1") exitWith {hint "Sie können das Objekt nicht ziehen";};
	if (_drag distance player > 4) exitWith {hint "Sie sind zuweit weg von dem Objekt!";};
	if (BTC_l_dragging) exitWith {hint "Sie können nicht mehr als ein Objekt ziehen!";};
	if (format ["%1", _drag getVariable "BTC_being_drag"] == "1") exitWith {hint "Sie können das Objekt nicht ziehen, da es schon gezogen wird!";};
	BTC_l_dragging = true;
...
BTC_l_placement =
{
	...
	if (format ["%1", _plac getVariable "BTC_cannot_place"] == "1") exitWith {hint "Sie können dieses Objekt nicht platzieren!";};
	...
	while {!BTC_l_end && Alive player} do
	{
		...
		hintSilent parseText "Keys:<br/>Mit W bewegt das Objekt in richtung Norden<br/>Mit A bewegt das Objekt in richtung Westem<br/>Mit S bewegt das Objekt in richtung Süden<br/>Mit D bewegt das Objekt in richtung Osten<br/>Mit Q und Z ändern Sie die höhe<br/>Mit ALT + A/D könnne Sie die richtung ändern<br/>Mit C können Sie die  Camera Öffen/Schliessen<br/>Mit CTRL + WASDQZ bewegen Sie die Camera<br/>Mit N benutzen Sie das Nachtsichtgereat<br/>Mit Back gehen Sie zurück ins Game";
		sleep 0.01;
	};
