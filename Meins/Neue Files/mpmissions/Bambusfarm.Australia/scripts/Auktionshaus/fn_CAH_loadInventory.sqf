...

{
	if(ITEM_VALUE(configName _x) > 0) then {
		_myInvListbox lbAdd format["%2 [x%1]",ITEM_VALUE(configName _x),localize (getText(_x >> "displayName"))];
		_myInvListbox lbSetData [(lbSize _myInvListbox)-1,configName _x];
		_icon = M_CONFIG(getText,"VirtualItems",configName _x,"icon");
		if(!(EQUAL(_icon,""))) then {
			_myInvListbox lbSetPicture [(lbSize _myInvListbox)-1,_icon];
		};
	};
