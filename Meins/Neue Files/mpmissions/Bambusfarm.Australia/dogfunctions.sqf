...
if(playerSide != west) exitWith {hint"Sie muessen ein Polizist sein um einen Polizei-Hund zu bekommen";};
if(life_cash < 40000) exitWith {hint"SIe haben nicht genug Geld, jeder Polizei-Hund braucht viel aufmerksamkeit!";};
...
_actions = 
	...
	while {(_unit getvariable "action")=="true"} do
		...
		if ((_unit getvariable "order") == "nil") then
			{
				_whistle = _unit addAction ["Rufen", _dogWhistle, [_unit, _dogGrowl,_playSound,_dogReturnIdle,_dogVehicle]];
		...
		if ((_unit getvariable "order") == "idle") then
			{
				_follow = _unit addAction ["<t color = '#ffff00'>Folgen</t>", _dogFollow, [_unit,_playSound]];
				_find = _unit addAction ["<t color = '#ffff00'>Suchen</t>", _dogSeek, [_unit,_playSound]];
				_rest = _unit addAction ["<t color = '#ff0000'>Weg schicken</t>", _dogHide, [_unit,_playSound]];
				_heel = _unit addAction ["<t color = '#ffff00'>Bei Fuss</t>", _dogHeel, [_unit,_playSound]];
				_unit setvariable ["step","wait"];
		...
		if ((_unit getvariable "order") == "active") then
			{
				_stop = _unit addAction ["<t color = '#ff0000'>Stopp!</t>", _dogStop, [_unit,_playSound]];
				_unit setvariable ["step","wait"];
