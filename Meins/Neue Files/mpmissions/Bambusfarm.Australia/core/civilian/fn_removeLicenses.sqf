...
switch (_state) do {
    ...
    //Remove licenses, because of many speedtickets
    case 4: {
        if(GVAR_MNS LICENSE_VARNAME("driver","civ") OR GVAR_MNS LICENSE_VARNAME("trucking","civ")) then {
            SVAR_MNS [LICENSE_VARNAME("driver","civ"),false];
            SVAR_MNS [LICENSE_VARNAME("trucking","civ"),false];
            hint localize "STR_Civ_LicenseRemoveSpeed";
        };
    };
};