...
_illegalValue = 0;
{
	...
	if(EQUAL(_isIllegalItem,1)) then{
		ADD(_illegalValue,(round(_val * _illegalPrice / 2)));
	};
...
if(_value > 0) then {
	...
	if (!isNull _vehicle) then {
		...
		hint "Sie oeffen gewaltsam den Kofferraum des Fahrzeuges!";
	} else {
		hint "Der Kofferraum ist blockiert!";
	};
	//end maximum code
