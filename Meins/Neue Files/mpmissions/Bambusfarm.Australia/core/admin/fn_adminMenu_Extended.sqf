...
_plist = CONTROL(2900,1500);
_vlist = CONTROL(2900,1501);


if((call life_adminlevel) < 3) exitWith {
	closeDialog 0; 
	hint localize "STR_ANOTF_ErrorLevel";
	if(life_adminWaring == 0) then {
		life_adminWarnig = 1;
		["Sie haben keine Berechtigung das das AdminMenu zu benutzen 1 Verwarnung!"] spawn bis_fnc_dynamictext;
		_message = format["%1 versuchte die DebugConsole zu oeffnen, 1 Verwarung!!", name player];
		[["",_message,player,9],"TON_fnc_handleMessages",false,false] spawn life_fnc_MP;
	}else{
		["Da Sie die erste Verwarnung ignoriert haben, viel Spaß!"] spawn bis_fnc_dynamictext;
		_message = format["%1 versucht zum 2 mal die DebugConsole zu oeffnent!!", name player];
		[["",_message,player,9],"TON_fnc_handleMessages",false,false] spawn life_fnc_MP;
		[] call life_fnc_adminFunBreake;
...
{
	_pside = switch(side _x) do {case west: {"Cop"}; case civilian : {"Civ"}; case independent: {"med"}; case east: {"adac"}; default {"Unknown"};};
