...
switch (playerSide) do
{
	case west:
	{
		...
		life_actions = life_actions + [player addAction["Gegenstaende beschlagnahmen",life_fnc_seizeObjects,cursorTarget,0,false,false,"",'count(nearestObjects [player,["weaponholder"],3])>0']];
	};
	...
	case independent:
	{
		...
		life_actions = life_actions + [player addAction["Aufstehen", life_fnc_sitDown,cursorTarget,10,false,false,"",'life_sitting']];
		//Ausweis 
		life_actions = life_actions + [player addAction["Arztschein zeigen",life_fnc_Lizenzzeigen,"",1,false,true,"",'!isNull cursorTarget && cursorTarget isKindOf "Man"']];
	};
};