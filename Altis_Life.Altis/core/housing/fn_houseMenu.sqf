...
private["_display","_curTarget","_Btn1","_Btn2","_Btn3","_Btn4","_Btn5","_Btn6","_Btn7","_Btn8","_Btn9","_uid","_house"];
if(!dialog) then {
...
if(_curTarget isKindOf "House_F" && playerSide == west) exitWith {
	if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _curTarget OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _curTarget) then {
		...
		if(!isNil {_curTarget getVariable "house_owner"}) then {
			...
			_Btn4 ctrlSetText "Zubehör";
			_Btn4 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_copsearchhouse; closeDialog 0;";
			_Btn4 ctrlShow true;
...
if(!(_curTarget in life_vehicles)) then {
	...
	_Btn2 ctrlSetText "Haus mieten";
	_Btn2 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_rentHouse;";
	_Btn2 ctrlShow true;
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		...
		_Btn1 ctrlSetText "Haus Menu";
		_Btn1 buttonSetAction "[life_pInact_curTarget] call life_fnc_creatdialogH;";
		_Btn1 ctrlShow true;
	};
};