...

{
	...
	if((typeOf _x) == "Land_fs_feed_F" && _x getVariable ["ga_id", -1] == -1) then {
		_x addAction["Befüllen",life_fnc_fuelStationMenu,["CAR",0.5],0,false,false,"","vehicle player == player && !life_action_inUse"];
	};
...
// Init all existing Fueltrucks as well
{
	if((typeOf _x) in life_fuelstation_cfgTrucks) then {
		_x addAction["Andere befüllen",life_fnc_fuelStationMenu,["FUELTRUCK",0.5],-1,false,false,"","vehicle player == player && !life_action_inUse && _target in life_vehicles"];
	};
} foreach vehicles;